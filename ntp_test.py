...
class NTPMonitor:
    ...
    @property
    def mean(self) -> float:
        return stats.mean([x.offset for x in self.data])
...
samples = NTPMonitor(1, 'time.google.com', 3, 50, .999999999999)
...
print(tableprint.row([samples.last.offset*1e+6, samples.status().delay, samples.mean*1e+6, samples.rms_offset, samples.stdev*1e+6, samples.tt_interval()[3]*1e6], width=width, style='clean'))
